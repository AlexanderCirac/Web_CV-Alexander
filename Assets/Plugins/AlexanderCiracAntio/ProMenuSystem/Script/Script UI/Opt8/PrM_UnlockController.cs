...
public static class PrM_Unlock
{
    ...
    public static void UnLockID(int _iD)
    {
        if ( !PrM_UIManager._instanceUIManager._listUnlockUI.Contains(_iD) )
            PrM_UIManager._instanceUIManager.ListOpt8(_iD);
    }
    ...
    public class PrM_UnlockController : MonoBehaviour
    {
        ...
        public List<int> _lockFinishLevel;
        #endregion
        ...
        private void OnDisable()
        {
            ...
            if( _lockFinishLevel != null || _lockFinishLevel.Count < 0 )
            {
                for ( int i = 0 ; i < _lockFinishLevel.Count ; i++ )
                {
                    PrM_Unlock.LockID(_lockFinishLevel[i]);
                }
            }
        }
        #endregion
