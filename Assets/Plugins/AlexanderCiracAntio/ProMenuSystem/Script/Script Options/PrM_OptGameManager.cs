...
namespace AlexanderCA.ProMenu.Opciones
{
    ...
    public class PrM_OptGameManager : PrM_Behaviour
    {
        ...
        void ToAddSounds()
        {
            ...
            if ( _audio.m_Sonido != null && _audio.m_Sonido.Length > 0 )
            {

                for ( int i = 0 ; i < _audio.m_Sonido.Length ; i++ )
                {
                    PrM_SoundInfo.Audioss _arraySound = _audio.m_Sonido[i];
                    //Add Sound option
                    if ( _arraySound._audioSource != null && _arraySound._audioSource.Length > 0 )
                    {
                        ...
                        if ( _data._soundFloat != null && _data._soundFloat.Count != _audio.m_Sonido.Length )
                        {
                            
                            if ( _audio.m_Sonido.Length > _data._soundFloat.Count )
                                _soundFloat.Add(_arraySound._soundFloat);

                        }
                        for ( int t = 0 ; t < _arraySound._audioSource.Length ; t++ )
                        {
                            if ( _arraySound._audioSource[t] == null )
                            {
                                contenido._option_1.GetErrorSound(i, t);
                            }
                        }
